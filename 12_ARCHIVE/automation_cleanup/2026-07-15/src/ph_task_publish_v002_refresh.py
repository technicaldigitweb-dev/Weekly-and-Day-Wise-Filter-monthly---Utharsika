"""
UAWSO v002 refresh publication (2026-07-13 data range).

Updates the EXISTING ph_task row (id=157) in place with the refreshed
v002 HTML content. Does NOT insert a new row, does NOT change
version_level (kept at 2 per instruction) - only html_content and
updated_at change.
...
print(f"Local refreshed v002 file: {len(v002_bytes)} bytes, SHA-256={v002_sha256}")
...
try:
    ...
    rows = cur.fetchall()
    print(f"Matching rows found: {len(rows)}")
    for r in rows:
        print(dict(r))
    ...
    row = rows[0]
    if row["id"] != EXPECTED_ROW_ID or row["task_id"] != EXPECTED_TASK_ID or row["version_level"] != 2:
        raise RuntimeError(
            f"Row identity/version mismatch: expected id={EXPECTED_ROW_ID} task_id={EXPECTED_TASK_ID} version_level=2, "
            f"found id={row['id']} task_id={row['task_id']} version_level={row['version_level']}. Refusing to write."
        )
    print("Row identity confirmed (version_level already 2 from prior v002 publish). Proceeding with UPDATE (html_content, updated_at only).")
    ...
    cur.execute(
        """
        SELECT id, task_name, task_id, version_level, version_status, updated_at, html_content
        FROM tech_team_outputs.ph_task WHERE id = %(row_id)s
        """,
    ...
    # Confirm max included date is 2026-07-13 by checking the stored content
    # embeds the expected history-end marker.
    marker_present = "2026-07-13" in after["html_content"]
    print(f"Stored content contains '2026-07-13' marker: {marker_present}")
