"""
Regeneration script for the sticky-header/sticky-columns/single-download/
Row-Type-removal/Column-Definitions UI update to the unpublished v004
report (REQ-02-D01 follow-up).
...
- does not re-query live data, so KPI figures cannot change as a result of
this UI-only update. Renders through the updated
templates/uawso_report_template_v5_asin_level.html via the unmodified
render_dashboard_v5() (only the template changed, not the renderer
function signature or the engine's calculation logic).

Writes:
    09_OUTPUTS\\staging\\2026-07-15_utharsika_v004_sticky_and_export_update.staging.html

Does not touch 09_OUTPUTS\\2026-07-15_utharsika_v004.html (final) or any
other existing file. Promotion to the final path is a separate, later step.
"""
...
MULTI_IMAGE_COUNT = 227  # same source-scope fact as the original v004 build (07_EVIDENCE\generated_data\2026-07-15_utharsika_v004_multi_image_selection.csv)
...
def main():
    ...
    html = dashboard_renderer.render_dashboard_v5(
        ...
        version=VERSION, template_version="5.1.0-sticky-single-download-no-rowtype",
    ...
    staging_path = os.path.join(STAGING_DIR, f"{RUN_DATE}_utharsika_{VERSION}_sticky_and_export_update.staging.html")
