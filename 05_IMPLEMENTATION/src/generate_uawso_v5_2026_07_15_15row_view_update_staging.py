"""
Regeneration script for the 15-row table-viewport correction to the
already-published v004 report (REQ-02-D01 follow-up).
...
render_dashboard_v5() (only the template's CSS changed - table viewport
height, not data/KPI logic).

Writes:
    09_OUTPUTS\\staging\\2026-07-15_utharsika_v004_15row_view_update.staging.html
...
def main():
    ...
    html = dashboard_renderer.render_dashboard_v5(
        ...
        version=VERSION, template_version="5.3.0-15row-viewport",
    ...
    staging_path = os.path.join(STAGING_DIR, f"{RUN_DATE}_utharsika_{VERSION}_15row_view_update.staging.html")
